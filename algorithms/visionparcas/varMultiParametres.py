...
class VarMultiParametres(QgsProcessingAlgorithm):
    """
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.NBPT_MIN,
            "Si polygone, nombre de points formant son contour (nombre minimum si variation)",
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        for count in liste_nbpoints:
            ...
            if typeGeom == 2:
                ...
                creationPoints = processing.run('Choucas:createptsaleatoires',
                {'INPUT': observedLayer,
                 'INPUT_COUNT' : count,
                 'OUTPUT': path_points},
                context=context, feedback=feedback, is_child_algorithm=True)
                ...
                #Si la couche en entrée est un point, il est ajouté seul à la liste de points
                    ...
                    for k in range (0, len(liste_path_mns)):
                        ...
                        if typeGeom == 2:
                            ...
                            #Initialisation d'une ligne:
                                resLigne = []
                                #Deuxième boucle: parcours de chaque case pour chaque ligne:
                                ...
                                resLigneFin = resLigne.copy()
                                #Ajout de la copie de la ligne à la liste du raster:
                                li_resultat.append(resLigneFin)
                            ...
                            #Transformer liste en numpy_array :
                            li_resultat2 = li_resultat.copy()
                            resArray = np.array(li_resultat2)
                            
                            res_output = dossierRaster + '/vpass_nbpt' + str(count) + '_Hcible' + str(int(liste_hauteurCible[i]*100)) + '_Hobs' + str(int(liste_hauteurObs[j]*100))
                            ...
                            resVisibPass = gdal.GetDriverByName('GTiff').Create(res_output, height, width, 1, gdal.GDT_UInt16)
                            resVisibPass.SetGeoTransform(geotransform2)
                            resVisibPass.SetProjection(srs2.ExportToWkt())
                            
                            #Ajout de la liste créé en tant que valeur pour chaque pixel
                            resVisibPass.GetRasterBand(1).WriteArray(resArray)
    ...
    def displayName(self):
        ...
        """
        return self.tr('Variation plusieurs paramètres - visibilité passive')
