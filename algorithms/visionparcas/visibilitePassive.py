...
class VisibilitePassive(QgsProcessingAlgorithm):
    """
    ...
    INPUT_HOBS = 'INTPUT_HOBS'
    INPUT_HCIBLE = 'INPUT_HCIBLE'
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.INPUT_HCIBLE,
            self.tr('Hauteur de la cible, meters'),
            QgsProcessingParameterNumber.Double,
            defaultValue= 0.0))
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        #Créer un dossier spécifique qui sera nommé selon la date:
        t = time.localtime()
        ...
        observers = self.parameterAsVectorLayer(parameters,self.OBSERVED_POINTS,context)       
        
        mnt = self.parameterAsRasterLayer(parameters,self.MNT, context)        
        mne = self.parameterAsRasterLayer(parameters,self.MNE, context)
        mns = self.parameterAsRasterLayer(parameters,self.MNS, context)
        
        Hobs = self.parameterAsDouble(parameters,self.INPUT_HOBS, context)
        hauteurCible = self.parameterAsDouble(parameters,self.INPUT_HCIBLE, context)
        ...
        """
        Test impact de operator : semble toujours valoir 1 :
        """
        operator = self.parameterAsInt(parameters,self.OPERATOR,context) + 1
        #operator = 1
        feedback.pushInfo("VALEUR DE OPERATOR :")
        feedback.pushInfo(str(operator))
        feedback.pushInfo(str(self.parameterAsInt(parameters,self.OPERATOR,context)))
        ...
        output_path_dbh = folderTempLayers+'/dbh.tif'
        #output_path_dbh = self.parameterAsOutputLayer(parameters,self.OUTPUT_DBH,context)
        feedback.pushInfo(str(output_path_dbh))
        
        #mns_path= mns.source()
        
        #Gestion des options : 
        if mns != None and mne != None:
            mns_path = mns.source()
        else:
            mns_path = mnt.source()
        
        dem = rst.Raster(mns_path, output=output_path_dbh)

        points = pts.Points(observers)
        
        #Modification : le champ observer_height devient target_height
        fields =["target_hgt", "radius"]
...
##                results[outputName] = parameters[outputName]
    ...
    def displayName(self):
        ...
        """
        return self.tr('xinterm - Calcul visibilité passive')
